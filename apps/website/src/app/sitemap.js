...
import { getPaperGroups } from '@/lib/papers/registry.mjs';
...
export default function sitemap() {
  const registryPages = [
    ...
    // The three level indexes, then one page per subject/exam/year group. Both
    // are generated from testPapers.mjs rather than the hub/spoke registry, so
    // lastModified never resolves for them.
    ...['primary', 'secondary', 'jc'].map((level) => ({
      path: `/free-test-papers/${level}`,
      priority: 0.6,
    })),
    ...getPaperGroups().map((g) => ({ path: g.url, priority: 0.5 })),
  ];

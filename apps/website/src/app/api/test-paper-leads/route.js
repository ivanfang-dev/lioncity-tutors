...
export async function POST(request) {
  ...
  try {
    ...
    const { email: rawEmail, phone: rawPhone, role: rawRole, subject, year, level, fileKey, paperKey, paperTitle } = data;
    ...
    let lead = await TestPaperLead.findOne({ email }).collation({ locale: 'en', strength: 2 });
    if (lead) {
